...
def getF1Score(name, variation):
    ...
    dfTrue = pd.read_csv('GoldStandard/{0}/{0}{1}True.txt'.format(name, variation), delimiter='\s+', index_col=False, encoding='utf-8', header=None).dropna()
    dfTrue.columns = ['word', 'tag']
    dfPred = pd.read_csv('GoldStandard/{0}/{0}{1}Out.txt'.format(name, variation), sep='\t', engine='python', encoding='utf-8', header=None).dropna()
    ...
    open('GoldStandard/{0}/{0}{1}Report.txt'.format(name, variation), "w").close()
    f = open('GoldStandard/{0}/{0}{1}Report.txt'.format(name, variation), 'w')
...
def copyFiles():
    ...
    for testCase in testCases:
        os.makedirs('Predictions/{}'.format(testCase))
        for variation1 in variations1:
            for variation2 in variations2:
                shutil.copy('GoldStandard/{0}/{0}{1}{2}.txt'.format(testCase, variation1, variation2), 'Predictions/{}/'.format(testCase), follow_symlinks=True)


def renameFiles():
    path = 'Predictions_BIOE1'
