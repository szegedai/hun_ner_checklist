...
def getF1Score(name, variation):
    ...
    dfTrue = pd.read_csv('GoldStandard/{0}/{0}{1}True.txt'.format(name, variation),
                         delimiter='\s+', index_col=False, header=None).dropna()
    dfTrue.columns = ['word', 'tag']

    dfPred = pd.read_csv('GoldStandard/{0}/{0}{1}Out.txt'.format(name, variation),
                         delimiter='\s+', index_col=False, header=None).dropna()
    ...
    for testCase in testCases:
        for variation in variations:
            ...
            os.system('cmd /c "java -Xmx3G -jar ner.jar -mode predicate '
                      '-input GoldStandard/{0}/{0}{1}In.txt '
                      '-output GoldStandard/{0}/{0}{1}Out.txt"'.format(testCase, variation))
    ...
    for testCase in testCases:
        ...
        for variation1 in variations1:
            for variation2 in variations2:
                shutil.copy('GoldStandard/{0}/{0}{1}{2}.txt'.format(testCase, variation1, variation2),
                            'Predictions/{}/'.format(testCase), follow_symlinks=True)
